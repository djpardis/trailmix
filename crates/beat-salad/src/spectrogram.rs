...
pub(crate) const FRAME_RATE: f32 = DEFAULT_SR as f32 / DEFAULT_HOP as f32;
...
pub fn log_mel_spectrogram(
    ...
    let mel_filterbank = mel_filters(n_mels, n_bins, DEFAULT_SR);
...
pub fn pick_peaks(activation: &[f32], threshold: f32, hop_size: usize) -> Vec<BeatPosition> {
    ...
    for i in window..activation.len().saturating_sub(window) {
        ...
        let time_seconds = i as f64 * hop_size as f64 / f64::from(DEFAULT_SR);
    ...
    assign_downbeats(&mut beats, activation, hop_size);
    beats
}

/// Assign `position_in_bar` (1-4) by finding the 4-beat grouping that maximizes
/// activation energy on the downbeat positions.
fn assign_downbeats(beats: &mut [BeatPosition], activation: &[f32], hop_size: usize) {
    if beats.len() < 4 {
    ...
    for phase in 0..4 {
        let energy: f32 = beats
            ...
            .step_by(4)
            .map(|b| {
                let frame = (b.time_seconds * f64::from(DEFAULT_SR) / hop_size as f64) as usize;
