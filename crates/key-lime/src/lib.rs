...
// Optional MLP-based key classifier. When weights are available (generated by
// benchmarks/train_mlp_key.py), this replaces profile correlation with a
// learned 12->48->24 network. Weights are baked in as const arrays so there
// is no file I/O or external dependency at runtime.
#[cfg(feature = "learned-key")]
mod mlp {
    use super::{Mode, MusicalKey, PitchClass};

    include!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/mlp_weights.rs"));

    pub fn classify_key_mlp(chroma: &[f32; 12]) -> (MusicalKey, f32, f32) {
        let mut hidden = [0.0_f32; HIDDEN_SIZE];
        for (h, bias) in hidden.iter_mut().zip(B1.iter()) {
            *h = *bias;
        }
        ...
        let mut logits = [0.0_f32; NUM_CLASSES];
        for (l, bias) in logits.iter_mut().zip(B2.iter()) {
            *l = *bias;
        }
