...
pub fn analyze(samples: &[f32], sample_rate: u32, config: KeyConfig) -> KeyAnalysis {
    ...
    while start + config.frame_size <= samples.len() {
        ...
        if frame_energy > 1.0e-8 {
            // Goertzel chroma (fast, good for higher frequencies)
            apply_window(frame, &window, &mut windowed_frame);
            let mut goertzel_chroma = [0.0; 12];
            accumulate_chroma_windowed(&windowed_frame, &goertzel_table, &mut goertzel_chroma);

            // CQT chroma (variable-length windows, better for low frequencies)
            let center = start + config.frame_size / 2;
            let cqt_chroma = cqt_table.chroma_at(samples, center);

            // Combine: normalize each, then average. If CQT has no energy
            // (e.g., no bass content in this frame), use Goertzel alone.
            let mut gc = goertzel_chroma;
            whiten_chroma(&mut gc);
            normalize_chroma(&mut gc);

            let cqt_energy: f32 = cqt_chroma.iter().sum();
            let frame_chroma = if cqt_energy > f32::EPSILON {
                let mut cc = cqt_chroma;
                whiten_chroma(&mut cc);
                normalize_chroma(&mut cc);
                let mut combined = [0.0f32; 12];
                for i in 0..12 {
                    combined[i] = 0.5 * gc[i] + 0.5 * cc[i];
                }
                normalize_chroma(&mut combined);
                combined
            } else {
                gc
            };
...
mod tests {
    ...
    fn detects_a_detuned_major_chord() {
        ...
        assert_eq!(
            ...
            "should detect A major despite 10-cent detuning, got {:?}",
            result.key
        );
    }
